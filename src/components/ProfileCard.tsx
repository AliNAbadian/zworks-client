...
const DEFAULT_SUNPILLAR_COLORS = [
  'hsl(2, 100%, 73%)',
  'hsl(53, 100%, 69%)',
  'hsl(93, 100%, 69%)',
  'hsl(176, 100%, 76%)',
...
interface ProfileCardProps {
  ...
  sunpillarColors?: readonly string[];
  nameGradient?: readonly [string, string];
  roleGradient?: readonly [string, string];
  shineSurface?: { base: string; mid: string; peak: string };
  glareColors?: { highlight: string; shadow: string };
...
const ProfileCardComponent: React.FC<ProfileCardProps> = ({
  ...
  const resolvedSunpillar = sunpillarColors ?? DEFAULT_SUNPILLAR_COLORS;
  ...
  const cardStyle = useMemo(
    () => ({
      ...
      '--sunpillar-1': resolvedSunpillar[0],
      '--sunpillar-2': resolvedSunpillar[1],
      '--sunpillar-3': resolvedSunpillar[2],
      '--sunpillar-4': resolvedSunpillar[3],
      '--sunpillar-5': resolvedSunpillar[4],
      '--sunpillar-6': resolvedSunpillar[5],
    ...
    [iconUrl, grainUrl, innerGradient, behindGlowColor, behindGlowSize, cardRadius, resolvedSunpillar]
  );
  ...
  const resolvedShineSurface = shineSurface ?? {
    base: '#0e152e',
    mid: 'hsl(180, 10%, 60%)',
    peak: 'hsl(180, 29%, 66%)'
  };

  const resolvedGlare = glareColors ?? {
    highlight: 'hsl(248, 25%, 80%)',
    shadow: 'hsla(207, 40%, 30%, 0.8)'
  };

  const resolvedNameGradient = nameGradient ?? ['#fff', '#6f6fbe'];
  const resolvedRoleGradient = roleGradient ?? ['#fff', '#4a4ac0'];
  ...
  const shineStyle = useMemo(
    () => ({
      ...
      repeating-linear-gradient(
        -45deg,
        ${resolvedShineSurface.base} 0%,
        ${resolvedShineSurface.mid} 3.8%,
        ${resolvedShineSurface.peak} 4.5%,
        ${resolvedShineSurface.mid} 5.2%,
        ${resolvedShineSurface.base} 10%,
        ${resolvedShineSurface.base} 12%
      ),
      radial-gradient(
    ...
    [cardRadius, resolvedShineSurface]
  );
  ...
  const glareStyle: React.CSSProperties = useMemo(
    () => ({
      ...
      farthest-corner circle at var(--pointer-x) var(--pointer-y),
      ${resolvedGlare.highlight} 12%,
      ${resolvedGlare.shadow} 90%
    )`,
      ...
      <div ref={shellRef} className="relative z-[1] group">
        <section
          ...
          style={{
            ...
            height: fillWidth ? 'auto' : (height ?? '80svh'),
            maxHeight: maxHeight ?? (fillWidth ? '480px' : '540px'),
            aspectRatio,
            borderRadius: cardRadius,
        ...
        >
          ...
          >
            ...
            >
              ...
              />
              {(showUserInfo || bottomContent) && (
                <div
                  className={cn(
                    'absolute z-[2] flex items-center backdrop-blur-[30px] border border-white/10 pointer-events-auto',
                    bottomContent ? 'justify-center' : 'justify-between',
                  )}
                  style={
                    {
                      '--ui-inset': '20px',
                      ...
                      padding: bottomContent ? '10px 12px' : '12px 14px'
              ...
              <div className="w-full absolute flex flex-col" style={{ top: '3em', display: 'flex', gridArea: 'auto' }}>
                <h3
                  ...
                  style={{
                    ...
                    backgroundImage: `linear-gradient(to bottom, ${resolvedNameGradient[0]}, ${resolvedNameGradient[1]})`,
                ...
                <p
                  ...
                  style={{
                    ...
                    fontSize: '16px',
                    margin: '0 auto',
                    backgroundImage: `linear-gradient(to bottom, ${resolvedRoleGradient[0]}, ${resolvedRoleGradient[1]})`,
                    backgroundSize: '1em 1.5em',
                    WebkitTextFillColor: 'transparent',
                    backgroundClip: 'text',
                    WebkitBackgroundClip: 'text',
